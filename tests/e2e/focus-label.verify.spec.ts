...
test('库存主数据布局与库位交互', async ({ page }, testInfo) => {
  ...
  for (const [, path, , title] of selectedPages) {
    ...
    if (path === 'batch') {
      await page.getByRole('button', { name: '办理入库' }).click()
      await page.getByRole('button', { name: '进入专注填单' }).click()
      const longLabel = page.locator('.art-form-item__label-text', {
        hasText: '单位成本（元）'
      })
      await expect(longLabel).toBeVisible()
      const metrics = await longLabel.evaluate((element) => ({
      ...
      await expect(page.locator('.el-popper[role="tooltip"]:visible').filter({ hasText: '单位成本（元）' })).toBeVisible()
      console.log('focus label metrics', metrics)
      await page.screenshot({ path: testInfo.outputPath('focus-label-dialog.png') })
    }
    if (path === 'bin') {
