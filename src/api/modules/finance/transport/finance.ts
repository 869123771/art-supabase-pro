...
type WaybillCost = Api.Finance.WaybillCostRecord
type WaybillCostSearchParams = Api.Finance.WaybillCostSearchParams
type WaybillOption = Api.Finance.WaybillOption
type WaybillOptionSearchParams = Api.Finance.WaybillOptionSearchParams
type CostReviewPayload = Api.Finance.CostReviewPayload
type ExpenseItem = Api.Finance.ExpenseItem
type ExpenseItemSearchParams = Api.Finance.ExpenseItemSearchParams
...
type OcrRun = Api.Finance.WaybillExpenseOcrRunRecord
type OcrRunSearch = Api.Finance.WaybillExpenseOcrRunSearchParams
type WaybillProfit = Api.Finance.WaybillProfitRecord
type WaybillProfitSearchParams = Api.Finance.WaybillProfitSearchParams
type FinanceWorkbenchStats = Api.Finance.FinanceWorkbenchStats
...
export async function fetchWaybillCostOverview() {
  const result = await responseHandle<Api.Finance.WaybillCostOverview>(
...
export async function analyzeWaybillCostByAi(
  costId: string
): Promise<QueryResult<Api.Finance.WaybillCostAuditResponse>> {
  const { data, error } = await supabase.functions.invoke<Api.Finance.WaybillCostAuditResponse>(
...
export async function fetchExpenseReimbursementDetail(id: string) {
  const [reimbursement, items] = await Promise.all([
    ...
    responseHandle<Api.Finance.ExpenseReimbursementItem[]>(
      () =>
        supabase
...
export async function createExpenseReimbursement(
  params: Api.Finance.CreateExpenseReimbursementPayload
) {
...
export async function executeExpenseReimbursement(
  params: Api.Finance.ExecuteExpenseReimbursementPayload
) {
  return await responseHandle<string>(
    () =>
...
export async function analyzeWaybillExpenseByAi(
  imageUrls: string[]
): Promise<QueryResult<Api.Finance.WaybillExpenseOcrAnalyzeResponse>> {
  const { data, error } =
    await supabase.functions.invoke<Api.Finance.WaybillExpenseOcrAnalyzeResponse>(
...
export async function analyzeWaybillProfitByAi(): Promise<
  QueryResult<Api.Finance.WaybillProfitAnalysisResponse>
> {
  const { data, error } =
    await supabase.functions.invoke<Api.Finance.WaybillProfitAnalysisResponse>(
...
export async function analyzeReceivablesCollectionByAi(): Promise<
  QueryResult<Api.Finance.ReceivablesCollectionResponse>
> {
  const { data, error } =
    await supabase.functions.invoke<Api.Finance.ReceivablesCollectionResponse>(
