...
type CashTransaction = Api.Finance.CashTransactionRecord
type CashTransactionSearchParams = Api.Finance.CashTransactionSearchParams
type AllocatableStatement = Api.Finance.CustomerStatementAllocatable
type AllocatableStatementSearchParams = Api.Finance.CustomerStatementAllocatableSearchParams
type CashAllocation = Api.Finance.CashAllocationRecord
type CreateReceiptPayload = Api.Finance.CreateCustomerReceiptPayload
type AllocateReceiptPayload = Api.Finance.AllocateCustomerReceiptPayload
type CarrierAllocatableStatement = Api.Finance.CarrierStatementAllocatable
type CarrierAllocatableSearchParams = Api.Finance.CarrierStatementAllocatableSearchParams
type CarrierCashAllocation = Api.Finance.CarrierCashAllocationRecord
type CreateCarrierPaymentPayload = Api.Finance.CreateCarrierPaymentPayload
type AllocateCarrierPaymentPayload = Api.Finance.AllocateCarrierPaymentPayload
...
export async function analyzeCashVoucherByAi(
  params: Api.Finance.CashVoucherOcrAnalyzeRequest
): Promise<QueryResult<Api.Finance.CashVoucherOcrAnalyzeResponse>> {
  const { data, error } =
    await supabase.functions.invoke<Api.Finance.CashVoucherOcrAnalyzeResponse>(
      'ai-cash-voucher-ocr',
      { body: params }
    )
...
export async function reviewCashVoucherOcrArtifact(
  params: Api.Finance.CashVoucherOcrReviewRequest
): Promise<QueryResult<Api.Finance.CashVoucherOcrReviewResponse>> {
  const { data, error } = await supabase.functions.invoke<Api.Finance.CashVoucherOcrReviewResponse>(
    'ai-cash-voucher-ocr',
    { body: params }
  )
  return { data: data ?? null, error: await normalizeSupabaseFunctionError(error) }
...
export async function analyzeBankStatementBatchByAi(params: {
  ...
  fileName: string
}): Promise<QueryResult<Api.Finance.BankBatchAnalyzeResponse>> {
  const { data, error } = await supabase.functions.invoke<Api.Finance.BankBatchAnalyzeResponse>(
...
export async function commitBankStatementBatchByAi(params: {
  artifactId: string
  rows: Api.Finance.BankBatchMatchRow[]
}): Promise<QueryResult<Api.Finance.BankBatchCommitResponse>> {
  const { data, error } = await supabase.functions.invoke<Api.Finance.BankBatchCommitResponse>(
    'ai-bank-statement-batch-match',
