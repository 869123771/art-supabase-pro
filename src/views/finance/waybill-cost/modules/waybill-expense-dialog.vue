...
<script setup lang="ts">
  ...
  type Waybill = Api.Finance.WaybillOption
  ...
  function filterSelectableExpenseItems(result: unknown): Api.Finance.ExpenseItem[] {
    const records = (result as { data?: Api.Finance.ExpenseItem[] })?.data ?? []
    const normalize = (
      items: Api.Finance.ExpenseItem[]
    ): Array<Api.Finance.ExpenseItem & { disabled?: boolean }> =>
      items
  ...
  function applyOcrResult(result: Api.Finance.WaybillExpenseOcrAnalyzeResponse): void {
