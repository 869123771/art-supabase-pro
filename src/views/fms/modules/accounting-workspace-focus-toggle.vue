<template>
  <div
    class="accounting-workspace-focus-toggle"
    :class="{ 'is-active': model }"
    title="隐藏概览与范围说明，只保留当前核算工作区；按 Esc 可退出"
  >
    <span class="accounting-workspace-focus-toggle__label">
...
<script setup lang="ts">
  defineOptions({ name: 'AccountingWorkspaceFocusToggle' })

  const model = defineModel<boolean>({ default: false })
</script>

<style scoped lang="scss">
  .accounting-workspace-focus-toggle {
    display: inline-flex;
    gap: 8px;
    align-items: center;
    min-height: 34px;
