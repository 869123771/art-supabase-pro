        ...
        <div>
          <div>
            ...
            <div class="flex items-start mb-3 last:mb-0">
              ...
              <ElTag :type="getRoleTagType(currentUser.userRoles?.[0])">
                {{ getRoleDisplayName(currentUser.userRoles?.[0]) }}
              </ElTag>
            </div>
            <div class="flex items-start mb-3 last:mb-0">
              ...
              <div class="flex flex-wrap gap-2">
                <ElTag
                  v-for="button in currentUserPermissions"
                  :key="button"
                  size="small"
                  type="info"
                >
                ...
                <span v-if="!currentUserPermissions.length" class="italic text-g-500"
                  >无权限码</span
                >
              </div>
            </div>
            ...
            <div class="mb-4">
              <div>
                ...
                <div class="flex flex-col gap-1">
                  ...
                  <span class="text-xs text-g-600">角色: {{ account.userRoles.join(', ') }}</span>
                </div>
              </div>
            </div>
...
<script setup lang="ts">
  ...
  const currentUserPermissions = computed(() => {
    const currentRole = currentUser.value.userRoles?.[0]
    return accounts.value.find((account) => account.userRoles[0] === currentRole)?.permissions ?? []
  })
