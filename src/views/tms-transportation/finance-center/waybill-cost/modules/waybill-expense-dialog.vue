...
<script setup lang="ts">
  ...
  defineOptions({ name: 'TmsWaybillExpenseDialog' })
  ...
  type Waybill = Api.Tms.Finance.WaybillOption
  ...
  function filterSelectableExpenseItems(result: unknown): Api.Tms.Finance.ExpenseItem[] {
    const records = (result as { data?: Api.Tms.Finance.ExpenseItem[] })?.data ?? []
    const normalize = (
      items: Api.Tms.Finance.ExpenseItem[]
    ): Array<Api.Tms.Finance.ExpenseItem & { disabled?: boolean }> =>
      items
  ...
  function applyOcrResult(result: Api.Tms.Finance.WaybillExpenseOcrAnalyzeResponse): void {
