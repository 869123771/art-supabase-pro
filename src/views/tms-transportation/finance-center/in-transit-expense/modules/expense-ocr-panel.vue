...
<script setup lang="ts">
  ...
  import { analyzeInTransitExpenseByAi } from '@/api/tms'

  defineOptions({ name: 'TmsInTransitExpenseOcrPanel' })

  const props = defineProps<{ modelValue: string[]; enabled: boolean }>()
  const emit = defineEmits<{
    'update:modelValue': [value: string[]]
    apply: [result: Api.Tms.Finance.InTransitExpenseOcrAnalyzeResponse]
    failed: []
  }>()

  const guideItems = [
    { icon: 'ri:money-cny-circle-line', title: '金额日期', description: '票面金额、消费时间' },
    { icon: 'ri:gas-station-line', title: '费用场景', description: '能源、充电、充气或其它' },
    { icon: 'ri:file-text-line', title: '票据要素', description: '服务商、票号、地点与数量' }
  ]
  const state = reactive<{
    analyzing: boolean
    result?: Api.Tms.Finance.InTransitExpenseOcrAnalyzeResponse
  ...
  async function handleAnalyze(): Promise<void> {
    ...
    try {
      const response = await analyzeInTransitExpenseByAi(imageUrls.value)
