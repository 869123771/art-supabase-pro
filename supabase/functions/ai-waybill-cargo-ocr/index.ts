...
function numberOrNull(value: unknown) {
  ...
  return Number.isFinite(normalized) && normalized > 0 ? Number(normalized.toFixed(3)) : null
}
...
async function loadFeatureConfig(admin: ReturnType<typeof createClient>, tenantId: string) {
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { error: accessError } = await userClient.rpc('tms_get_waybill_cargo_operation_context', {
    ...
    const model =
      config?.vision_model || config?.model || Deno.env.get('OPENAI_MODEL') || 'gpt-4.1-mini'
