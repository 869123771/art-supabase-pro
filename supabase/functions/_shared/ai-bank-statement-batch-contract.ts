...
export function inferAiBankBatchMapping(
  headers: string[]
): Partial<Record<AiBankBatchColumn, string>> {
...
export function normalizeAiBankBatchRows(
  rows: Record<string, unknown>[],
  context: AiBankBatchContext
) {
  return rows.map((source, index) => {
    ...
    const direction: AiCashVoucherDirection | null =
      explicitDirection ??
    ...
    const amount = Math.abs(
      explicitAmount ?? (direction === 'receipt' ? receiptAmount : paymentAmount) ?? 0
    )
    ...
    const counterpartyName =
      text(valueAt(source, context.mapping, 'counterpartyName')) ??
    ...
    const counterparty = direction
      ? matchCounterparty(counterpartyName, direction, context.counterparties)
      : null
    const matches = direction
      ? matchAiCashVoucherStatements(
          ...
          direction,
          context.statementCandidates.filter((item) => item.counterpartyId === counterparty?.id)
        )
      : []
    ...
    if (bankReference && context.existingReferences.has(`${direction}:${bankReference}`))
      issues.push('银行流水号已入账')
    ...
    return {
      ...
      allocations:
        bestMatch && bestMatch.score >= 65
          ? [
              {
                statementId: bestMatch.statementId,
                allocationAmount: bestMatch.recommendedAllocation
              }
            ]
          : [],
      issues
