...
export function assessVehicleHealth(
  ...
  if (insuranceDays) {
    const signal = createComplianceSignal('insurance', insuranceDays.remaining, insuranceDays.value)
  ...
  const latestMileage =
    latestBy(mileage, 'end_time', 'endTime') ?? latestBy(mileage, 'start_time', 'startTime')
