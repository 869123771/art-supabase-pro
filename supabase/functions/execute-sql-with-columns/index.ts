...
function mapPostgresTypeToJSType(pgType?: string | null): string {
  ...
  if (
    ...
    return 'number'
  if (t.includes('char') || t.includes('text') || t === 'uuid' || t === 'inet' || t === 'cidr')
    return 'string'
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!authHeader) {
      ...
      return json(
        { status: 'error', message: 'Missing authorization header' } satisfies SqlExecuteResponse,
        401
      )
    }
    ...
    if (authError || !user) {
      ...
      return json(
        { status: 'error', message: 'Invalid or expired token' } satisfies SqlExecuteResponse,
        401
      )
    }
    ...
    if (superError) {
      ...
      return json(
        {
          status: 'error',
          message: 'Failed to verify SQL console permission'
        } satisfies SqlExecuteResponse,
        500
      )
    }
    ...
    if (queryText.includes(';')) {
      ...
      return json(
        {
          status: 'error',
          message: "Multiple statements are not allowed (unexpected ';').",
          query_text: queryText
        } satisfies SqlExecuteResponse,
        400
      )
    ...
    if (isWriteOperation && !isSuper) {
      const permissionMessage =
        'Permission denied: write and DDL statements are restricted to platform super administrators'
      await writeAudit({
        auth_user_id: user.id,
        auth_email: user.email ?? null,
        query_text: queryText,
    ...
    const {
      ...
      error,
      status: rpcStatus
    } = await supabaseAdmin.rpc('execute_sql_query', {
      sql_query: queryText
    })
    ...
    if (error) {
      ...
      return json(
        {
          status: 'error',
          message: formatErrorMessage(pgMessage),
          query_text: queryText,
          duration_ms: durationMs
        } satisfies SqlExecuteResponse,
        rpcStatus || 500
