...
begin
  ...
  if new.statement_type = 'cash_flow_statement' then
    if new.item_code in ('CF010','CF100','CF200') then
      new.calculation_method := 'label';
    elsif new.item_code in ('CF099','CF199','CF299','CF399') then
      new.calculation_method := 'formula';
    end if;
  end if;
