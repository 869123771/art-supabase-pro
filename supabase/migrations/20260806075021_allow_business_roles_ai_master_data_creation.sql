-- AI one-click archive is a permission-equivalent batch operation: only roles
-- that can already maintain TMS business data may execute it. The function
-- remains SECURITY INVOKER so table grants, tenant RLS and audit triggers stay
-- authoritative for every inserted record.
...
begin
  if not (select app_private.can_manage_tms()) then
    raise exception '当前角色无权执行 AI 主数据建档';
  end if;
