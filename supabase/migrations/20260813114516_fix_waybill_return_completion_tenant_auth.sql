...
BEGIN
  ...
  IF NOT app_private.can_manage_tms()
     OR (
       ...
       AND v_order.tenant_id IS DISTINCT FROM app_private.current_user_tenant_id()
     ) THEN
      ...
      OR (
        ...
        AND NOT EXISTS (
          ...
          WHERE e.waybill_id = v_waybill.id
            ...
            AND app_private.tms_valid_url_array(e.return_photo_urls)
        )
      )
  );
END;
...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    WHERE e.waybill_id = NEW.id
      ...
      AND app_private.tms_valid_url_array(e.departure_photo_urls)
      AND e.signed_at IS NOT NULL
      AND NULLIF(btrim(e.signer_name), '') IS NOT NULL
      AND app_private.tms_valid_url_array(e.signature_urls)
      AND e.return_time IS NOT NULL
      AND e.return_odometer_km IS NOT NULL
      AND app_private.tms_valid_url_array(e.return_photo_urls)
...
BEGIN
  ...
  IF NOT app_private.tms_valid_url_array(p_return_photo_urls) THEN
  ...
  IF v_waybill.status = 'completed'
     ...
     AND app_private.tms_valid_url_array(v_execution.return_photo_urls) THEN
  ...
  IF NOT (
    ...
    OR (
      ...
      AND v_waybill.tenant_id = app_private.current_user_tenant_id()
      AND (
        v_waybill.driver_id = app_private.current_user_driver_id()
  ...
  INSERT INTO public.tms_waybill_event (
    ...
    jsonb_build_object(
      ...
      'source', CASE
        WHEN v_waybill.driver_id = app_private.current_user_driver_id()
          THEN 'driver'
        ELSE 'web'
      END,
...
BEGIN
  ...
  IF NOT (
    ...
    OR v_waybill.driver_id = app_private.current_user_driver_id()
  ) THEN
  ...
  v_needs_return_completion :=
    ...
    AND NOT (
      ...
      AND v_execution.return_odometer_km IS NOT NULL
      AND app_private.tms_valid_url_array(v_execution.return_photo_urls)
