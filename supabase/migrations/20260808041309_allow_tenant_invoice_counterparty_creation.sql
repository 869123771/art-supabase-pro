-- Reuse the same tenant-scoped RLS write boundary as the standard
-- customer/carrier create screens. The function remains SECURITY INVOKER,
-- so authenticated users can create only where their existing table grants
-- and RLS policies already allow a manual insert.
...
begin
  ...
  if v_status is null then
    v_status := 'unmatched';
    v_can_create := v_normalized_name <> ''
      and char_length(v_name) between 2 and 100;
    v_message := case
      when v_can_create then '未找到现有往来单位，请核对识别信息后建档并带入'
      else '未找到现有往来单位；请补全单位名称后建档，或手动选择'
    end;
  end if;
...
is 'Tenant-scoped resolution of an OCR invoice party, including whether the current RLS boundary permits a confirmed create attempt.';

comment on function public.create_tms_invoice_counterparty_from_ocr(uuid, text, text, text)
is 'Tenant-scoped SECURITY INVOKER creation or reuse of an OCR invoice customer/carrier record, guarded by existing table RLS.';
