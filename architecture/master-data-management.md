...
当前版本提供租户隔离的治理总览和统一主数据管理中心，覆盖 8 个查询工作区、24 类独立主档定义，并辅助查看 2 类关系/明细对象（员工任职、客户地址）。目录支持服务端分页、关键字/来源/生命周期/资料质量筛选、来源追溯和安全详情查看。来源业务应用仍承担新增、修改、停用等写操作；等写契约和数据责任完成迁移后，再逐域把维护入口收敛到 MDM，避免过渡期双写。
...
- 24 类独立主档纳入统一目录：组织、职族、职级、职务、岗位、员工、往来主体、客户、承运商、供应商、保险公司、外部服务商、站点、货物、司机、车辆、设备/备件及其分类、物料及其分类、场所和存放位置。
- `mdm_customer_address` 和 `mdm_employee_assignment` 作为关系/明细对象辅助治理查看，不计入 24 类独立主档；`mdm_business_partner_role` 是内部角色映射，不作为客户端目录对象。这些数据继续在所属主档或来源业务流程中管理。
